...
class LangSplitterByMetadata(TextSplitter):
    ...
    def _load_splitters(self) -> None:
        self._splitter_map = {}
        for lang in self._lang_ext_map.lang_ext_map.keys():
            try:
                self._splitter_map[lang] = self._get_lang_splitter(lang)
            except ValueError:
                # Fall back to default for unsupported languages (e.g. PERL)
                self._splitter_map[lang] = RecursiveCharacterTextSplitter(
